...
input,textarea{width:100%;padding:12px;border:1px solid #d8c0ad;border-radius:12px;background:#fffdfa;color:#4e3425;font-size:15px}
textarea{resize:vertical;min-height:78px;font-family:inherit}
input:focus,textarea:focus{outline:none;border-color:#a56a43;box-shadow:0 0 0 3px rgba(165,106,67,.16)}
...
void initFriendlyNamePortal() {
  ...
  server.on("/config", HTTP_GET, [](AsyncWebServerRequest *r) {
    ...
    String j = "{\"dn\":\"" + jsonEsc(cfg.deviceName) +
               "\",\"device_name\":\"" + jsonEsc(cfg.deviceName) +
               "\",\"c1\":\"" + jsonEsc(cfg.call1) +
               "\",\"c2\":\"" + jsonEsc(cfg.call2) +
               "\",\"c3\":\"" + jsonEsc(cfg.call3) +
               "\",\"sms\":\"" + jsonEsc(cfg.smsTemplate) +
               ...
               "\",\"home\":" +
               String((cfg.homeLat != 0 || cfg.homeLng != 0) ? "true"
                                                              : "false") +
               ",\"home_lat\":" + String(cfg.homeLat, 6) +
               ",\"home_lng\":" + String(cfg.homeLng, 6) +
               ",\"geo_en\":" +
               String(cfg.geofenceEnable ? "true" : "false") +
