...
static bool doWiFiGeolocation() {
  ...
  // Simple parser — avoid pulling in ArduinoJson
  double lat = 0, lng = 0;
  float accuracy = 9999;

  int latIdx = response.indexOf("\"lat\"");
  int lngIdx = response.indexOf("\"lng\"");
  if (lngIdx < 0) lngIdx = response.indexOf("\"lon\""); // unwiredlabs uses lon
  ...
  // Find : after "lat", then parse number
  ...
  if (lat == 0.0 && lng == 0.0) {
    logLine("[NETLOC] Parsed 0,0 — invalid");
    return false;
  }
...
static bool doCellGeolocationCLBS() {
  ...
  if (lat == 0.0 && lng == 0.0) {
    logLine("[NETLOC] CLBS returned 0,0 — invalid");
    return false;
  }
...
void networkLocationTask(void *pvParameters) {
  ...
  while (true) {
    ...
    // 1) Cell geolocation first: more realistic for field deployments
    if (!success) {
      success = doCellGeolocationCLBS();
    }

    // 2) WiFi geolocation only as a secondary fallback
    if (!success) {
